...
const ServerDetailsBlock = () => {
    ...
    const diskLimit = limits.disk ? megabytesToHuman(limits.disk) : '無限制';
    const memoryLimit = limits.memory ? megabytesToHuman(limits.memory) : '無限制';
    const cpuLimit = limits.cpu ? limits.cpu + '%' : '無限制';
    ...
    return (
        <TitledGreyBox css={tw`break-words`} title={name} icon={faServer}>
            <p css={tw`text-xs uppercase`}>
                ...
                />
                &nbsp;{!status ? '連接中...' : (isInstalling ? '安裝中' : (isTransferring) ? '轉移中' : status)}
                {stats.uptime > 0 &&
            ...
            <p css={tw`text-xs mt-2`}>
                <FontAwesomeIcon icon={faSitemap} fixedWidth css={tw`mr-1`}/> 所在節點: {node}
            </p>
            <p css={tw`text-xs mt-2`}>
            ...
            <p css={tw`text-xs mt-2`}>
                <FontAwesomeIcon icon={faMemory} fixedWidth css={tw`mr-1`}/> 記憶體: {bytesToHuman(stats.memory)}
                <span css={tw`text-neutral-500`}> / {memoryLimit}</span>
            </p>
            <p css={tw`text-xs mt-2`}>
                <FontAwesomeIcon icon={faHdd} fixedWidth css={tw`mr-1`}/> 儲存空間: {bytesToHuman(stats.disk)}
                <span css={tw`text-neutral-500`}> / {diskLimit}</span>
            </p>
            <p css={tw`text-xs mt-2`}>
                <FontAwesomeIcon icon={faDatabase} fixedWidth css={tw`mr-1`}/> 資料庫: {databases.length} 已使用
                <span css={tw`text-neutral-500`}> / {databaseLimit} 最多</span>
            </p>
            <p css={tw`text-xs mt-2`}>
                <FontAwesomeIcon icon={faArchive} fixedWidth css={tw`mr-1`}/> 備份: {backups.pagination.total} 已使用
                <span css={tw`text-neutral-500`}> / {backupLimit} 最多</span>
            </p>
            <p css={tw`text-xs mt-2`}>
                <FontAwesomeIcon icon={faEthernet} fixedWidth css={tw`mr-1`}/>
