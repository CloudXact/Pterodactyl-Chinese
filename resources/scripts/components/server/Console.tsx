...
export default () => {
    const TERMINAL_PRELUDE = '\u001b[1m\u001b[33m容器@面板~ \u001b[0m';
    ...
    const handleTransferStatus = (status: string) => {
        switch (status) {
            ...
            case 'failure':
                terminal.writeln(TERMINAL_PRELUDE + '傳輸已失敗\u001b[0m');
                return;

            // Sent by the source node whenever the server was archived successfully.
            case 'archive':
                terminal.writeln(TERMINAL_PRELUDE + '伺服器已成功壓縮，正在連線至傳送節點..\u001b[0m');
        }
    };
    ...
    const handlePowerChangeEvent = (state: string) => terminal.writeln(
        TERMINAL_PRELUDE + '伺服器已標為 ' + state + '...\u001b[0m',
    );
