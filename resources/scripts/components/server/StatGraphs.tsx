...
export default () => {
    ...
    return (
        <div css={tw`mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4`}>
            <TitledGreyBox title={'記憶體使用量'} icon={faMemory}>
                {status !== 'offline' ?
                    <canvas
                        id={'memory_chart'}
                        ref={memoryRef}
            ...
            <TitledGreyBox title={'CPU使用量'} icon={faMicrochip}>
                {status !== 'offline' ?
            ...
            <TitledGreyBox title={'輸入資料'} icon={faEthernet}>
                {status !== 'offline' ?
            ...
            <TitledGreyBox title={'輸出資料'} icon={faEthernet}>
                {status !== 'offline' ?
