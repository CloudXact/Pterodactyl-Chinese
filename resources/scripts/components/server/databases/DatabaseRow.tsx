...
export default ({ database, className }: Props) => {
    ...
    const schema = object().shape({
        confirm: string()
            .required('請提供資料庫名稱。')
            .oneOf([ database.name.split('_', 2)[1], database.name ], '請提供資料庫名稱。'),
    });
    ...
    return (
        <>
            ...
            >
                {
                    ({ isSubmitting, isValid, resetForm }) => (
                        ...
                        >
                            ...
                            <h2 css={tw`text-2xl mb-6`}>你確定要刪除資料庫嗎?</h2>
                            <p css={tw`text-sm`}>
                                刪除資料庫是永久性的操作，無法撤消。這將永久
                                刪除 <strong>{database.name}</strong> 資料庫並移除所有相關資料。
                            </p>
            ...
            <Modal visible={connectionVisible} onDismissed={() => setConnectionVisible(false)}>
                ...
                <h3 css={tw`mb-6 text-2xl`}>資料庫連接資訊</h3>
                <div>
                    <Label>端點</Label>
            ...
            <GreyRowBox $hoverable={false} className={className} css={tw`mb-2`}>
                ...
                <div css={tw`ml-8 text-center hidden md:block`}>
                    ...
                    <p css={tw`mt-1 text-2xs text-neutral-500 uppercase select-none`}>允許的連接</p>
                </div>
                <div css={tw`ml-8 text-center hidden md:block`}>
                    <CopyOnClick text={database.username}><p css={tw`text-sm`}>{database.username}</p></CopyOnClick>
                    <p css={tw`mt-1 text-2xs text-neutral-500 uppercase select-none`}>用戶名稱</p>
                </div>
                <div css={tw`ml-8`}>
